...
export class RcsbLineDisplay extends RcsbCoreDisplay implements RcsbDisplayInterface{
    ...
    line:Line<[number,number]> = line().curve(curveStep);
    ...
    setInterpolationType(type: string): void{
        if(type === INTERPOLATION_TYPES.CARDINAL)
            ...
            this.line = line().curve(curveStep);
        else if(type === INTERPOLATION_TYPES.BASIS)
            this.line = line().curve(curveBasis);
    ...
    private setFunction(): void{
        ...
        this.line
            ...
            .y(function (d:any) {
                return self._height - self.yScale(d.val);
            });
    }

    updateFunction(): void{
        const self: RcsbLineDisplay = this;
        this.line.x(function (d: any) {
            return self.xScale(d.pos);
        });
    }
    ...
    downSampling(points: any[]):any[] {
        ...
        if(out.length>thr) {
            ...
            sampler.value((d:any) => {return d.val});
            sampler.bucketSize(bucketSize);
            const all: any[] = sampler(points);
