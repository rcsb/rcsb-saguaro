...
export class RcsbD3SequenceManager implements RcsbD3DisplayManagerInterface{
    ...
    plot(config: PlotSequenceInterface){
        ...
        config.elements
            ...
            .attr(RcsbD3Constants.Y, function (d, i) {
                return yScale(Math.floor(config.height*0.5)+4);
            })
            .style(RcsbD3Constants.TEXT_ANCHOR, "middle")
            .style(RcsbD3Constants.FILL, (d) => {
    ...
    private static opacity (elems: Selection<SVGGElement,any,BaseType,undefined>, xScale: ScaleLinear<number,number>, intervalRatio: [number,number]): void {
